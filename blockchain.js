...
class Blockchain {
  ...
  generateNextBlock(blockData) {
    ...
    let nonce = 0;
    let nextHash = "";
    while (!this.hashMatchesDifficulty(nextHash)) {
      nonce = nonce + 1;
  ...
  isValidNewBlock(newBlock, previousBlock) {
    ...
    if (previousBlock.index + 1 !== newBlock.index) {
      ...
      !this.hashMatchesDifficulty(this.calculateHashForBlock(newBlock))
    ) {
      console.log(
  ...
  // Ham thay the phuc vu viec dong bo giua cac node voi nhau
